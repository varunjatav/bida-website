...
$sql = "SELECT SQL_CALC_FOUND_ROWS T1.ID, T1.TenderTitle, T1.TenderRefrenceNo, T1.ReleaseDate, T1.PreBidMeeting, T1.BidSubmission, T1.TechnicalPresentation, T1.TenderDescription, T1.TenderStatus, T1.BidOpeningPreQualificationCheck, T1.FinancialBidOpening, T1.Active, T1.DateCreated, GROUP_CONCAT(T2.FileName SEPARATOR '@@') AS FileName, GROUP_CONCAT(T2.Description SEPARATOR '@@') AS Description, GROUP_CONCAT(T2.Attachment SEPARATOR '@@') AS Attachment, GROUP_CONCAT(T2.Date SEPARATOR '@@') AS Dated
                          FROM bida_tender T1
